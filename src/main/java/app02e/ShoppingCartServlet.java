...
public class ShoppingCartServlet extends HttpServlet{
    ...
    public void init() throws ServletException{
        products.add(new Product(1,"Brove 32","Low-cost HDTV from renowned TV manufacturer",159.95F));
        products.add(new Product(2,"Bravo BluRay Player","High quality stylish bluRay player",99.95F));
        products.add(new Product(3,"Bravo Stereo System","5 speaker hifi system with ipod player",129.59F));
        products.add(new Product(4,"Bravo ipod player","An ipod plug-in that can lay multiple formats",39.95F));
    }
    ...
    private void sendProductDetails(HttpServletRequest request,HttpServletResponse response)throws IOException{
        ...
        if (product != null){
            ...
            writer.println("<tr><td><input name='quantity'/></td>"
                    +"<td><input type='submit' value='Buy'/></td></tr>");
            writer.println("<tr><td colspan='2'>"
                    +"<a href='products'>Product List</a>"
                    +"</td></tr>");
            writer.println("</table></form></body>");
        }else {
        ...
        if(cart != null){
            ...
            for (ShoppingItem shoppingItem : cart){
                ...
                if(quantity != 0){
                    ...
                    writer.println("<td>"+currencyFormat.format(product.getPrice())+"</td>");
